...
glm::vec3 PhongMaterial::EvaluateReflectedEnergy(const Intersection &isx, const glm::vec3 &outgoing_ray, const glm::vec3 &incoming_ray,  Integrator& integrator, unsigned int integrator_depth)
{
    ...
    //First, we calculate Lambert term
    float diffuseTerm = glm::dot(n, glm::normalize(-light_direction));

    diffuseTerm = clamp(diffuseTerm, 0, 1);

    //Second, we calculate specularity term
    glm::vec3 mirror_ray_driection_against_normal = incoming_ray - 2.0f * glm::dot(incoming_ray, n) * n;

    float specularityTerm = pow(glm::dot(glm::normalize(eye_direction),
                                glm::normalize(mirror_ray_driection_against_normal)),
                                specular_power);

    specularityTerm = clamp(specularityTerm, 0, 1);


    //Finally, we add things together
    glm::vec3 basicReflectedEnergy  = light_color * k_diffuse * real_base_color * diffuseTerm +
                                      light_color * k_specular * specularityTerm;
    ...
    if(reflectivity != 0){
            ...
            mirror_ray_driection_against_normal = -outgoing_ray - 2.0f * glm::dot(glm::normalize(-outgoing_ray), glm::normalize(n)) * glm::normalize(n);
